...
import { filterTextNotes } from '../../../utils/noteFilters';
...
export default function NotasIndexScreen() {
  ...
  const emptyState = getListEmptyState('note', allNotes.length, searchQuery);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.listWrapper}>
        ...
        <VirtualizedList<TextNote>
          estimatedItemSize={120}
          data={filteredNotes}
          renderItem={({ item }) => (
            <NoteListItem
              item={item}
              onPress={() => router.push(`${TAB_ROUTES.note}/${item.id}`)}
            />
          )}
